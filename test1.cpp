...
public:
    ...
    void get_other_pub(int dest_id) {
    ...
    void send_sym(int dest_id) {
        aes::key_t sym;
        aes::key_gen(sym);
        rsa::key_t ids_pub;
        if (!get_pub_by_id_from_fs(ids_pub, dest_id)) {
            std::cout << "no ids pub" << std::endl;
        }
    ...
    void send_message(int dest_id, const std::string& message) {
        aes::key_t sym;
        ...
        std::unique_ptr<Packet> req = std::make_unique<MessageRequest>(dest_id, chiphered_message);
        send_request(*req);
    }
